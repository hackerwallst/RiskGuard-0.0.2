...
def run_daemon(mt5_path: str, poll_s: int = 3, cal_refresh_min: int = 10):
    ...
    # --- FIX: respeitar kill-switch ao iniciar o daemon ---
    from limits.kill_switch import kill_status, _toggle_autotrade
    
    status = kill_status()

    if status["active"]:
        debug(f"[KILL SWITCH] Kill ativo ao iniciar. Bloqueado até {status['until']}.")
        _toggle_autotrade(False)
        debug("[KILL SWITCH] AutoTrading forçado para OFF na inicialização.")

        while True:
            status = kill_status()

            if not status["active"]:
                debug("[KILL SWITCH] Kill expirou. Prosseguindo com daemon…")
                break

            debug("[KILL SWITCH] Kill ainda ativo ao iniciar. Aguardando…")
            time.sleep(5)
    ...
    while True:
        ...
        if state["until"] is not None:
            ...
            if reenabled:
                ...
                debug("[NEWS_DEBUG] AutoTrading reativado. Forçando reconexão MT5…")
                ...
                if not init_ok:
                    debug("[NEWS_DEBUG] Falha ao inicializar MT5. Tentando novamente…")
                    time.sleep(2)
                    mt5.shutdown()
                    time.sleep(1)
                    mt5.initialize()
                ...
                attempts = 0
                while reader.snapshot() is None:
                    attempts += 1
                    debug(f"[NEWS_DEBUG] MT5 voltou mas sem resposta ({attempts}). Reconnect…")
                    mt5.shutdown()
                    time.sleep(1)
                    mt5.initialize()
                    time.sleep(1)
                    ...
                    if attempts >= 5:
                        debug("[NEWS_DEBUG] MT5 demorando demais, seguindo mesmo assim.")
                        break

                debug("[NEWS_DEBUG] Reconexão completa. Continuando leitura normal.")
